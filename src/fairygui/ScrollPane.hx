...
class ScrollPane extends EventDispatcher
{
    ...
    public var pageController(get, set):Controller;
                        ...
                        hzScrollBarRes:String)
    {
        ...
        if (scrollBarDisplay != ScrollBarDisplayType.Hidden)
        {
            if (_scrollType == ScrollType.Both || _scrollType == ScrollType.Vertical)
            {
                ...
                if (res != null)
                {
                    _vtScrollBar = try cast(UIPackage.createObjectFromURL(res), GScrollBar)
            ...
            if (_scrollType == ScrollType.Both || _scrollType == ScrollType.Horizontal)
            {
                ...
                if (res != null)
                {
                    _hzScrollBar = try cast(UIPackage.createObjectFromURL(res), GScrollBar)
                    catch (e:Dynamic) null;
    ...
    public function get_pageController():Controller
    {
        return _pageController;
    }

    public function set_pageController(value:Controller):Controller
    {
    ...
    @:allow(fairygui)
    private function handleControllerChanged(c:Controller):Void
    {
